...
The program should now begin running. It will flash its light for a few seconds, then it will connect to the Switch and navigate its way back into the game. After a brief pause, it will start mashing A. To stop the program, simply unplug the Arduino at any time.
...
Follow the [hardware guide](Hardware-ArduinoUnoR3.md) all the way to the end. (all 3 sections)

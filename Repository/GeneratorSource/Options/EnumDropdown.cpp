    ...
    : SingleStatementOption(obj)
{
    ...
    for (const auto& option : options){
        if (!option.isArray()){
            throw "Config Error - Expected Array: " + JSON_OPTIONS;
        }
        QJsonArray pair = option.toArray();
        if (pair.size() != 2){
            throw "Config Error - Enum pairs should be 2 elements: " + JSON_OPTIONS;
        }
    ...
    for (size_t c = 0; c < m_options.size(); c++){
        if (!m_map.emplace(m_options[c].first, c).second){
            throw "Config Error - Duplicate option token.";
        }
    }
    {
        auto iter = m_map.find(json_get_string(obj, JSON_DEFAULT));
        if (iter == m_map.end()){
            throw "Config Error - Unrecognized token.";
        }
